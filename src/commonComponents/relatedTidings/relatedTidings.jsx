        ...
        <div className='related_tidings_subcontainer'>
            ...
            <div className='related_tidings_blogs_widget_section'>
            {
              blogsData?.map((item,index)=>{
                if (item._id !== id) {
               return <NavLink style={{textDecoration:'none'}} key={index+1} to={`/blogs/${item._id}`}><BlogWidget key={item._id} image={item.blogImage} title={item.blogTitle} date={item.createdAt} alt='blog_img' /></NavLink>
              }
              })
            }
            </div>
